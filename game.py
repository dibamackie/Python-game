...
from a2_partc import UndoFeature # import the undo class 
...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites, undo_feature):
        self.width = width
        self.height = height
        ...
        self.turn = 0
        self.undo_feature = undo_feature
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            self.undo_feature.save_state(self.get_board()) # save the current state of the game
...
status=["",""]
current_player = 0
undo_feature = UndoFeature() # initialize the undo feature 
board = Board(GRID_SIZE[1], GRID_SIZE[0], p1_sprites, p2_sprites, undo_feature)
# Game loop
running = True
...
grid_row = -1
choice = [None, None]
undo_button = pygame.Rect(900, 200, 200, 50) # creating a button  for undo

while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == pygame.MOUSEBUTTONDOWN: # this happens when a playes click a button with it mouse 
            x, y = event.pos #check where mouse cllicked 
            if undo_button.collidepoint(x, y): #check if undo button clicked 
                if undo_feature.undo_last_move(board): # undo the last move
                    current_player = (current_player + 1) % 2 #switching playes after undo 
    ...
    window.blit(undo_text, (undo_button.x + 50, undo_button.y + 10))
